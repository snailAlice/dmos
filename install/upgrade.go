...
func ExitUpgradeCase(version, pkgUrl, cfgFile string) error {
	...
	if err := upgradeDmos.Load(cfgFile); err != nil {
		upgradeDmos.ShowDefaultConfig()
		return err
	}
...
func (u *DmosUpgrade) upgradeNodes(hostnames []string, isMaster bool) {
	wg := NewPool(2)
	var err error
