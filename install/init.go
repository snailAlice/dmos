...
import (
	...
	"github.com/snailAlice/dmos/k8s"
...
func (s *DmosInstaller) getCgroupDriverFromShell(h string) string {
	var output string
	if For120(Version) {
		cmd := ContainerdShell
...
func (s *DmosInstaller) appendApiServer() error {
...
func (s *DmosInstaller) CreateKubeconfig() {
	...
	err := cert.CreateJoinControlPlaneKubeConfigFiles(cert.DmosConfigDir,
...
func (s *DmosInstaller) InstallMaster0() {
	...
	// remote server run dmos init . it can not reach apiserver.cluster.local , should add masterip apiserver.cluster.local to /etc/hosts
	...
	configYamlDir := filepath.Join(home, ".dmos", "cni.yaml")
...
func (s *DmosInstaller) to11911192(masters []string) (to11911192 bool) {
