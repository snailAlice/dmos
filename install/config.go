...
func (c *SealConfig) Dump(path string) {
	...
	if err != nil {
		logger.Warn("create default dmos config dir failed, please create it by your self mkdir -p /root/.dmos && touch /root/.dmos/config.yaml")
	}
...
func (c *SealConfig) ShowDefaultConfig() {
	...
	c.CertPath = home + "/.dmos/pki"
	c.CertEtcdPath = home + "/.dmos/pki/etcd"
	...
	logger.Info("\n\n%s\n\n", string(y))
	logger.Info("Please save above config in ~/.dmos/config.yaml and edit values on your own")
}
