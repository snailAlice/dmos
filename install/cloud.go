...
func newCommand(c *Cluster) string {
	...
	releaseURL := fmt.Sprintf("https://github.com/snailAlice/dmos/releases/download/v%s/dmos%s_linux_amd64.tar.gz",
		version, version)
	cmd := fmt.Sprintf("wget %s -O -| tar -xz && chmod +x dmos", releaseURL)
	cmd += fmt.Sprintf(" && ./dmos init --passwd %s --pkg-url %s --version %s", c.Passwd, getLocalURL(c.Version), c.Version)
