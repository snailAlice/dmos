...
var exampleCmd = `
 	# snapshot save the etcd, the backupPath is on etcd nodes. not on the dmos init machine.
	dmos etcd save --name snapshot --backupPath  /opt/dmos/etcd-backup
	
	# save the snapshot to aliyun oss bucket. this recommend to use in cronjob 
	dmos etcd save --aliId youraliyunkeyid --aliKey youraliyunkeysecrets --ep oss-cn-hangzhou.aliyuncs.com  \
	--bucket etcdbackup  --objectPath /dmos/ --docker

	# snapshot restore the etcd
	dmos etcd restore --name snapshot --backupPath  /opt/dmos/etcd-backup
...
func NewEtcdCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:    `dmos etcd save --name snapshot`,
		Example: exampleCmd,
	}
...
func NewEtcdSaveCommand() *cobra.Command {
	...
	cmd.Flags().StringVar(&install.ObjectPath, "objectPath", "", "aliyun oss objectPath to save snapshot, like: /dmos/snapshots/")
