...
import (
	...
	"github.com/snailAlice/dmos/net"
...
var exampleInit = `
	...
	dmos init --passwd your-server-password  \
...
var initCmd = &cobra.Command{
	...
	Long: `dmos init --master 192.168.0.2 --master 192.168.0.3 --master 192.168.0.4 \
	--node 192.168.0.5 --user root --passwd your-server-password \
	--version v1.18.0 --pkg-url=/root/kube1.18.0.tar.gz`,
	Example: exampleInit,
...
func init() {
	...
	// 不像用户暴露
	// initCmd.Flags().StringVar(&install.CertPath, "cert-path", cert.GetUserHomeDir() + "/.dmos/pki", "cert file path")
	// initCmd.Flags().StringVar(&install.CertEtcdPath, "cert-etcd-path", cert.GetUserHomeDir() + "/.dmos/pki/etcd", "etcd cert file path")
}

func NewInitGenerateCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "gen",
		Short: "show default dmos init config",
		Run: func(cmd *cobra.Command, args []string) {
			c := &install.SealConfig{}
			c.ShowDefaultConfig()
		},
