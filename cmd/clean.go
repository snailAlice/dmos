...
var exampleCleanCmd = `
	# clean  master
	dmos clean --master 192.168.0.2 \
	--master 192.168.0.3
  
	# clean  node  use --force/-f will be not prompt 
	dmos clean --node 192.168.0.4 \
	--node 192.168.0.5 --force

	# clean master and node
	dmos clean --master 192.168.0.2-192.168.0.3 \
...
func CleanCmdFunc(cmd *cobra.Command, args []string) {
	...
	err := c.Load(cfgFile)
	if err != nil {
		// comment: if cfgFile is not exist; do not use dmos clean something.
		// its danger for dmos do clean nodes without `~/.dmos/config.yaml`
		//// 判断错误是否为配置文件不存在
	...
	// 使用 dmos clean --node   不小心写了 masterip.
	if ok, node := deleteOrJoinNodeIsExistInCfgNodes(deleteNodes, c.Masters); ok {
		logger.Error(`clean master Use "dmos clean --master %s" to clean it, exit...`, node)
		os.Exit(-1)
	}
	// 使用 dmos clean --master 不小心写了 nodeip.
	if ok, node := deleteOrJoinNodeIsExistInCfgNodes(deleteMasters, c.Nodes); ok {
		logger.Error(`clean nodes Use "dmos clean --node %s" to clean it, exit...`, node)
		os.Exit(-1)
	}
