#!/bin/bash
# sh test.sh [k8s version] [package url] [dmos version]
# sh test.sh 1.15.4 pkgurl 3.1.0
# 如测试3.2.0-beta.2 版本 1.18.0版本离线包:
# sh test-dmos-HA.sh 1.18.0 https://sealyun.oss-cn-beijing-internal.aliyuncs.com/d551b0b9e67e0416d0f9dce870a16665-1.18.0/kube1.18.0.tar.gz 3.2.0-beta.2
...
remotecmd "wget https://sealyun.oss-cn-beijing-internal.aliyuncs.com/v$3/dmos && chmod +x dmos && mv dmos /usr/bin"

version=$1
pkgurl=$2

echo "test init one master"
remotecmd "dmos init --master $master0  \
...
remotecmd "dmos join --master $master1"
echo "wait for everything ok"
sleep 40
...
remotecmd "dmos clean --master $master1"
echo "wait for everything ok"
sleep 40
...
remotecmd "dmos join --master $master1 --master $master2"
echo "wait for everything ok"
sleep 40
...
remotecmd "dmos join --node $node0"
echo "wait for everything ok"
sleep 40
...
remotecmd "dmos join --node $node1 --node $node2"
echo "wait for everything ok"
sleep 40
...
remotecmd "dmos clean --node $node0"
echo "wait for everything ok"
sleep 40
...
remotecmd "dmos clean --node $node1 --node $node2"
